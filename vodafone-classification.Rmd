...
```{r bagging}
fit <- bagging(as.factor(TARGET) ~ diff_one + diff_two + diff_three + AVG_PER_SESSION + TOTAL_SESSIONS, data = train)
calculate(fit)
```

```{r decision tree}
fit <- J48(as.factor(TARGET) ~ diff_one + diff_two + diff_three + AVG_PER_SESSION + diff_one2 + diff_two2 + TOTAL_USAGE, data = train)
calculate(fit)
```

```{r random forest}
fit <- randomForest(as.factor(TARGET) ~ diff_one + diff_two + diff_three + AVG_PER_SESSION + diff_one2 + diff_two2 + TOTAL_USAGE, data = train, ntree=50)
calculate(fit)
```
