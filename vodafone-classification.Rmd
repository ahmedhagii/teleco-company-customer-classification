...
# Submission 15:
Outliers are bad for some of the classification models, while others are resistant to such outliers. Tree based models are not affected by outliers that much, because they split the data base on equations like “if predictor A is greater
than X, predict the class to be Y”. However, let's try to remove the outliers and see if that will affect our naiveBayes model.
```{r}
