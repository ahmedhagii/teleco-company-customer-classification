...
```{r naive bayes}
fit <- naiveBayes(as.factor(TARGET) ~ TOTAL_USAGE + AVG_PER_SESSION + SUM_EXCEEDING + WEIGHTED_AVG, data = train)
calculate(fit)
```

```{r bagging}
fit <-  bagging(as.factor(TARGET) ~ TOTAL_USAGE + AVG_PER_SESSION + SUM_EXCEEDING + WEIGHTED_AVG, data = model_train, type='class')
calculate(fit)
```
